...
**Answer 1:**
In the example above, the priors are normally distributed, and pulled from a random distribution based on the previous randomly drawn value. Instead, I will keep the priors normally distributed, but the distributions will be described using these functions:

- dnorm(proposed_pars[1], mean = 3, sd = 1, log = TRUE) 
- dnorm(proposed_pars[2], mean = 0.5, sd = 0.25, log = TRUE)

The mean and standard deviation used in the functions to describe the distribution of the parameters were determined from the distribution of parameters in the example. It produced reasonable values, and by starting close to it's output I hope to save computation time. Additionally, an optim log likelihood method could be used to determine reasonable starting points for parameters for the prior distribution.
...
```{r}
h1 <- rnorm(seq(1, 100, 1), mean = 3, sd = 0.05)
h2 <- rnorm(seq(1, 100, 1), mean = 0.5, sd = 0.05)
...
**Answer 5:**
It took 2 iterations for my first parameter, and ~10 parameters for my second parameter. I think that the burn-in was so short because of my informed means and standard deviations from the exmaple run of this model.  
...
for(i in 1:num_samples){
  ...
  pred_posterior_mean[i, ] <- d$par1[sample_index] * (x_new / (x_new + d$par2[sample_index]))
