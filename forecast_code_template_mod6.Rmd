...
weather_past_mean <- weather_past |> 
...
weather_forecast_s3 <- neon4cast::noaa_stage2(start_date = as.character(noaa_date))

weather_forecast <- weather_forecast_s3 |> 
  dplyr::filter(datetime >= forecast_start_date,
                site_id %in% lake_sites$field_site_id,
                variable %in% met_variables) |> 
  collect()

weather_forecast_daily <- weather_forecast |> 
  mutate(datetime = as_date(datetime)) |> 
...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  weather_forecast_site <- weather_forecast_daily |> 
    filter(site_id == curr_site)
  ...
  for (t in 1:length(forecasted_dates)) {
    ...
    temp_driv <- weather_forecast_site %>%
    ...
    curr_site_df <- tibble(datetime = rep(forecasted_dates[t], times = n_members),
                           site_id = curr_site,
