...
ens_members <- 100
params <- list()
params$alpha <- rep(0.02, ens_members) #light use efficiency, governs 
params$SLA <- rep(4.74, ens_members) #specific leaf area
params$leaf_frac <- rep(0.315, ens_members)
params$Ra_frac <- rep(0.5, ens_members)
params$Rbasal <- rep(0.002, ens_members) #baseline respiration rate of the soil
params$Q10 <- rep(2.1, ens_members) #sensitivity
...
plot_partitioned_uc_highlight <- function(sd_df, x){
  ...
  ggplot() +
    geom_bar(data = sd_df, aes(x = horizon, y = variance, fill = model_id), 
             stat = "identity", position = "stack") +
  geom_bar(data = filter(sd_df, horizon != x), 
           aes(x = horizon, y = variance, fill = model_id), 
           stat = "identity", position = "stack",
           fill = "grey")+
    ylab("Variance (MgC/ha/day^2 or MgC/ha^2)") +
...
```
  ...
  Answer: The largest source of variance 1 day ahead for a flux is driver uncertainty, and for a state it is initial condition uncertainty.
...
For both fluxes and states, the individually incorporated uncertainty results in a lower variance in forecast predictions, while combining the uncertainty before running the forecast results in higher variance, especially as the forecast horizon increases. They do not match, and this shows that when incorporating uncertainty it is important to do it all at once. Doing it all individually and then adding it may not capture how the different sources of uncertainty propagate in the model, and end up amplifying each others influence.
